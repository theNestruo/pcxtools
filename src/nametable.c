...
void nameTableProcessorInit(NameTableProcessor *this, int argc, char **argv) {
  ...
  if ((this->generateBlank =
           ((i = argStartsWith(argc, argv, "-bb", 4)) != -1))) {
...
void nameTableProcessorGenerate(NameTableProcessor *this, NameTable *nametable,
                                Charset *charset) {
  ...
  if (this->traverseHorizontally) {
    ...
    for (i = 0, b = 0, blockCount = 0, src = dest = charset->blocks,
        it = nametable->namtbl;
    ...
    for (x = 0, b = 0, blockCount = 0, src = dest = charset->blocks,
        it = nametable->namtbl;
         x < charset->width; x++) {
    ...
    for (i = 0; i < nametable->namtblSize; i++) {
      int toX = i / charset->height, toY = i % charset->height,
  ...
  if (this->generateBlank && (this->blankAt <= charset->blockCount)) {
    ...
    // Blank
    Line *itLine;
    for (i = 0, itLine = charset->blocks[this->blankAt].line; i < TILE_HEIGHT;
         i++, itLine++) {
      itLine->pattern = 0x00;
      itLine->color = 0x00;
    }
...
void nameTableProcessorGenerateUsing(__attribute__((unused))
                                     NameTableProcessor *this,
  ...
  int i, j, *it;
  Block *block;
  for (i = 0, it = nametable->namtbl, block = screen->blocks;
       i < nametable->namtblSize; i++, it++, block++) {
...
void nameTableProcessorApplyTo(NameTable *nametable, Charset *charset) {
  ...
  for (i = expected = 0, blockCount = 0, src = dest = charset->blocks,
      it = nametable->namtbl;
       (i < charset->blockCount) && (i < nametable->namtblSize);
       i++, src++, it++) {
...
void nameTableProcessorPostProcess(NameTableProcessor *this,
                                   NameTable *nametable) {
  ...
  for (i = 0, it = nametable->namtbl; i < nametable->namtblSize; i++, it++)
    *it = (*it == -1)
              ...
              : *it + this->namtblOffset +
                    ((this->generateBlank) && (*it >= this->blankAt) ? 1 : 0);
}

int nameTableProcessorWrite(NameTableProcessor *this, NameTable *nametable,
                            FILE *namFile) {
...
void reallocateBlocks(Charset *charset, int newBlockCount) {
  ...
  if (newBlockCount) {
    ...
    charset->blocks =
        (Block *)realloc(charset->blocks, newBlockCount * sizeof(Block));
  } else {
    // Remove blocks
    free(charset->blocks);
    charset->blocks = NULL;
