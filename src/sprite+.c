...
void sprWriterPlusInit(SprWriterPlus *this, int argc, char **argv) {
    ...
    this->offsetX =
        ((i = argStartsWith(argc, argv, "-x", 2)) != -1) ? decimalInt(&(argv[i][2])) : this->spriteWidth / 2;
    this->offsetY =
        ((i = argStartsWith(argc, argv, "-y", 2)) != -1) ? decimalInt(&(argv[i][2])) : this->spriteHeight / 2;
...
void solve(SpriteSolver *this) {
    ...
    for (color = 1; color < 16; color++) {
        ...
        int solutionIndex, i;
        Rect *rect;
        for (solutionIndex = this->solutionIndexes[color], i = 0; i < colorSolver->rectsPerSolution;
             i++, solutionIndex++) {
...
int stSpriteSolverWrite(SpriteSolver *this, int *spriteIndex, FILE *sprFile, FILE *spatFile) {
    ...
    for (color = 1, sprite = buffer; color < 16; color++) {
        ...
        for (solutionIndex = this->solutionIndexes[color], j = 0; j < colorSolver->rectsPerSolution;
...
int writePadding(SpriteSolver *this, FILE *spatFile) {
    ...
    return (this->cfg->binaryOutput)
               ? (fwrite(paddingBuffer, sizeof(uint8_t), this->cfg->attributePadding, spatFile) ==
                  (size_t)this->cfg->attributePadding)
               : (asmComment(spatFile, "padding", 1) &&
                  asmBytes(spatFile, paddingBuffer, this->cfg->attributePadding) && asmNewLine(spatFile));
}
...
void addLayer(Rect **rects, int *count, Rect *src) {
    ...
    (*count)++;
    (*rects) = (*rects) ? (Rect *)realloc((*rects), (*count) * sizeof(Rect)) : (Rect *)calloc((*count), sizeof(Rect));
...
int writePattern(Sprite *this, SprWriterPlus *cfg, FILE *sprFile) {
    ...
    return (cfg->binaryOutput) ? fwrite(this->pattern, sizeof(uint8_t), 32, sprFile) == 32
                               : asmBytes(sprFile, this->pattern, 16) && asmBytes(sprFile, &(this->pattern[16]), 16);
}

int writeAttributes(Sprite *this, SprWriterPlus *cfg, FILE *spatFile) {

    return (cfg->binaryOutput) ? (fwrite(this->attributes, sizeof(uint8_t), 4, spatFile) == 4)
