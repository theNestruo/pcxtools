...
void processSpriteGroup(SprWriter *instance, SpriteGroup *group, Bitmap *bitmap,
                        int x, int y);
...
void sprWriterInit(SprWriter *this, int argc, char **argv) {
  ...
  // Read arguments
  this->spriteWidth = (argEquals(argc, argv, "-8") != -1)   ? 8
...
void sprWriterReadSprites(SprWriter *this, Bitmap *bitmap) {
  ...
  this->groupCount = ((int)(bitmap->width / this->spriteWidth)) *
  ...
  if (this->traverseHorizontally) {
    for (y = 0, it = this->groups; (y + this->spriteHeight) <= bitmap->height;
         y += this->spriteHeight) {
      for (x = 0; (x + this->spriteWidth) <= bitmap->width;
           x += this->spriteWidth, it++) {
        processSpriteGroup(this, it, bitmap, x, y);
      }
    }
  } else {
    for (x = 0, it = this->groups; (x + this->spriteWidth) <= bitmap->width;
         x += this->spriteWidth) {
      for (y = 0; (y + this->spriteHeight) <= bitmap->height;
           y += this->spriteHeight, it++) {
        processSpriteGroup(this, it, bitmap, x, y);
      }
    }
  }
...
int sprWriterWrite(SprWriter *this, FILE *sprFile) {
  ...
  for (i = 0, it = this->groups; i < this->groupCount; i++, it++) {
    ...
    for (j = 0, sprite = it->sprites; j < it->spriteCount; j++, sprite++) {
      if (fwrite(sprite->pattern, sizeof(uint8_t), spriteSize, sprFile) !=
          spriteSize)
        return 2;
    }
  }
...
void processSpriteGroup(SprWriter *this, SpriteGroup *group, Bitmap *bitmap,
                        int x0, int y0) {
  ...
  // Create the pattern
  int col, y, x;
  for (col = 0, j = 0; col < this->spriteWidth;
       col += 8) {                                  // for each 8px width column
  ...
  if (!group->spriteCount) {
    ...
    const unsigned int colorOrderHighLow[] = {
        0xF - 1, 0xE - 1, 0xD - 1, 0xC - 1, 0xB - 1, 0xA - 1, 0x9 - 1, 0x8 - 1,
        0x7 - 1, 0x6 - 1, 0x5 - 1, 0x4 - 1, 0x3 - 1, 0x2 - 1, 0x1 - 1};
    const unsigned int colorOrderLowHigh[] = {
        0x1 - 1, 0x2 - 1, 0x3 - 1, 0x4 - 1, 0x5 - 1, 0x6 - 1, 0x7 - 1, 0x8 - 1,
        0x9 - 1, 0xA - 1, 0xB - 1, 0xC - 1, 0xD - 1, 0xE - 1, 0xF - 1};
    const unsigned int colorOrderLightDark[] = {
        0xF - 1, 0xE - 1, 0xB - 1, 0xA - 1, 0x7 - 1, 0x3 - 1, 0x9 - 1, 0xD - 1,
        0x5 - 1, 0x8 - 1, 0x2 - 1, 0xC - 1, 0x6 - 1, 0x4 - 1, 0x1 - 1};
    const unsigned int colorOrderDarkLight[] = {
        0x1 - 1, 0x4 - 1, 0x6 - 1, 0xC - 1, 0x2 - 1, 0x8 - 1, 0x5 - 1, 0xD - 1,
        0x9 - 1, 0x3 - 1, 0x7 - 1, 0xA - 1, 0xB - 1, 0xE - 1, 0xF - 1};
