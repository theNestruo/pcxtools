...
int tiledWrite(Tiled *this, FILE *binFile) {
  ...
  for (y = 0, yMax = (int)(this->height / this->metatileSize),
      groupSrc = this->data;
       y < yMax; y++, groupSrc += this->width * this->metatileSize) {
    for (x = 0, xMax = (int)(this->width / this->metatileSize),
        tileSrc = groupSrc;
         x < xMax; x++, tileSrc += this->metatileSize) {
      for (v = 0, src = tileSrc; v < this->metatileSize;
           v++, src += this->width) {
