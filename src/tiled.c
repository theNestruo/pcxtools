...
void tiledOptions() {
    ...
    // implemented) printf("\t-m\tmirrored. Flips bitmap horizontally\n"); // (not
    // yet implemented)
    printf("\t-t<0..255>\treorganize data as metatiles of <0..255>x<0..255> "
           "bytes\n");
}
...
void tiledInit(Tiled *this, int argc, char **argv) {
    ...
    // Read arguments
    // this->isFlip = (argEquals(argc, argv, "-i") != -1); // (not yet
    // implemented) this->isMirror = (argEquals(argc, argv, "-m") != -1); // (not
    // yet implemented)
...
int tiledWrite(Tiled *this, FILE *binFile) {
    ...
    if (this->metatileSize == 1) {
        return (fwrite(this->data, this->width * this->height, 1, binFile) != 1) ? 1 : 0;
    }

    // Metatile rearrangment or mirrored
    int x, xMax, y, yMax, v;
    uint8_t *groupSrc, *tileSrc, *src;
    for (y = 0, yMax = (int)(this->height / this->metatileSize), groupSrc = this->data; y < yMax;
         y++, groupSrc += this->width * this->metatileSize) {
        for (x = 0, xMax = (int)(this->width / this->metatileSize), tileSrc = groupSrc; x < xMax;
             x++, tileSrc += this->metatileSize) {
            for (v = 0, src = tileSrc; v < this->metatileSize; v++, src += this->width) {
