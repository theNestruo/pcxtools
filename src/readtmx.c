...
void tmxReaderOptions() { printf("\t-b\tmultibank charset support\n"); }
...
int tmxReaderRead(TmxReader *this, FILE *file, Tiled *tiled) {
    ...
    if ((!(height = readProperty(line, "height"))) || (!(width = readProperty(line, "width"))) ||
        (!(name = readProperty(line, "name")))) {
        printf("ERROR: Invalid <layer>: missing properties.\n");
        i = 8;
        goto out;
    }
    if ((!(tiled->width = atoi(width))) || (!(tiled->height = atoi(height)))) {
    ...
    for (y = 0; y < tiled->height; y++) {
        ...
        int x, val;
        char *token;
        for (x = 0, token = strtok(line, ","); x < tiled->width; x++, token = strtok(NULL, ","), dest++) {
            if (!token) {
