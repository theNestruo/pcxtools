...
int startsWith(char *string, char *prefix) {
    ...
    return (prefixLength <= stringLength) ? (strncmp(prefix, string, prefixLength) == 0) : 0;
}
...
int endsWith(char *string, char *suffix) {
    ...
    return (suffixLength <= stringLength) ? (strncmp(suffix, string + stringLength - suffixLength, suffixLength) == 0)
                                          : 0;
}
...
int decimalDigit(char c) { return ((c >= '0') && (c <= '9')) ? (c - '0') : 0; }
